...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const players = {}; const teamCar = {};
    stats.forEach((r) => { const w = parseInt(r.week || "0", 10); if (w < minW) return; const k = (TM[r.team] || r.team) + "|" + w; teamCar[k] = (teamCar[k] || 0) + (f(r.carries) || 0); });
    stats.forEach((r) => { const w = parseInt(r.week || "0", 10); if (w < minW) return; const k = norm(r.player_display_name) + "|" + r.position; const rec = players[k] = players[k] || { team: TM[r.team] || r.team, wk: {} };
      rec.wk[w] = { opp: TM[r.opponent_team] || r.opponent_team, snap: snaps[k + "|" + w] != null ? Math.round(snaps[k + "|" + w] * 100) : null, tgt: f(r.targets), ts: f(r.target_share) != null ? Math.round(f(r.target_share) * 100) : null, ays: f(r.air_yards_share) != null ? Math.max(0, Math.round(f(r.air_yards_share) * 100)) : null, rs: teamCar[(TM[r.team] || r.team) + "|" + w] ? Math.round(((f(r.carries) || 0) / teamCar[(TM[r.team] || r.team) + "|" + w]) * 100) : 0, rec: f(r.receptions), recy: f(r.receiving_yards), car: f(r.carries), ry: f(r.rushing_yards), att: f(r.attempts), py: f(r.passing_yards), td: (f(r.passing_tds) || 0) + (f(r.rushing_tds) || 0) + (f(r.receiving_tds) || 0), ptd: f(r.passing_tds) || 0, int: f(r.passing_interceptions) || 0, rtd: f(r.rushing_tds) || 0, rectd: f(r.receiving_tds) || 0, fl: (f(r.rushing_fumbles_lost) || 0) + (f(r.receiving_fumbles_lost) || 0) + (f(r.sack_fumbles_lost) || 0), cmp: f(r.completions) || 0, epa: Math.round(((f(r.passing_epa) || 0) + (f(r.rushing_epa) || 0) + (f(r.receiving_epa) || 0)) * 10) / 10, wopr: f(r.target_share) != null && f(r.air_yards_share) != null ? Math.max(0, Math.round((1.5 * f(r.target_share) + 0.7 * Math.max(0, f(r.air_yards_share))) * 100) / 100) : null, racr: f(r.racr), yac: f(r.receiving_yards_after_catch), adot: f(r.receiving_air_yards) != null && f(r.targets) ? Math.round((f(r.receiving_air_yards) / f(r.targets)) * 10) / 10 : null, rz: null,
        fp: Math.round(((f(r.passing_yards) || 0) / 25 + (f(r.passing_tds) || 0) * 4 - (f(r.passing_interceptions) || 0) * 2 + (f(r.rushing_yards) || 0) / 10 + (f(r.rushing_tds) || 0) * 6 + (f(r.receptions) || 0) * 0.5 + (f(r.receiving_yards) || 0) / 10 + (f(r.receiving_tds) || 0) * 6 - ((f(r.rushing_fumbles_lost) || 0) + (f(r.receiving_fumbles_lost) || 0) + (f(r.sack_fumbles_lost) || 0)) * 2 + ((f(r.passing_40) || 0) + (f(r.rushing_40) || 0) + (f(r.receiving_40) || 0)) * 2 + ((f(r.receiving_yards) || 0) >= 100 ? 3 : 0) + ((f(r.rushing_yards) || 0) >= 100 ? 3 : 0) + ((f(r.passing_yards) || 0) >= 300 ? 3 : 0)) * 10) / 10, p40: f(r.passing_40) || 0, r40: f(r.rushing_40) || 0, c40: f(r.receiving_40) || 0 }; });
