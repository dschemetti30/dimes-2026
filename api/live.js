...
// Hogg Heaven: half PPR, 4-pt pass TD, +2 per 40+ yard completion, run or catch (approximated from the box-score long), +3 for 100+ receiving yards; 40+ yard TD bonuses need play-by-play and are added Monday from nflverse
function scoreLine(l) { return Math.round((l.py / 25 + l.ptd * 4 - l.int * 2 + l.ry / 10 + l.rtd * 6 + l.rec * 0.5 + l.recy / 10 + l.rectd * 6 - l.fl * 2 + l.fgpts + l.xp + (l.long >= 40 ? 2 : 0) + (l.rlong >= 40 ? 2 : 0) + (l.clong >= 40 ? 2 : 0) + (l.recy >= 100 ? 3 : 0) + (l.ry >= 100 ? 3 : 0) + (l.py >= 300 ? 3 : 0)) * 100) / 100; }
      ...
      try {
        ...
        (s.boxscore && s.boxscore.players || []).forEach((tp) => { const T = TM[(tp.team && tp.team.abbreviation) || ""] || (tp.team && tp.team.abbreviation) || ""; (tp.statistics || []).forEach((cat) => { const labels = cat.labels || []; const idx = (l) => labels.indexOf(l);
          (cat.athletes || []).forEach((a) => { const name = a.athlete && a.athlete.displayName; if (!name) return; const k = norm(name); const st0 = a.stats || []; const g = (l) => (idx(l) >= 0 ? st0[idx(l)] : null);
            ...
            if (cat.name === "passing") { const [c, at] = frac(g("C/ATT")); rec.att = at; rec.py = num(g("YDS")); rec.ptd = num(g("TD")); rec.int = num(g("INT")); rec.long = 0; }
            if (cat.name === "rushing") { rec.car = num(g("CAR")); rec.ry = num(g("YDS")); rec.rtd = num(g("TD")); rec.rlong = num(g("LONG")); }
            if (cat.name === "receiving") { rec.rec = num(g("REC")); rec.recy = num(g("YDS")); rec.rectd = num(g("TD")); rec.tgt = num(g("TGTS")); rec.clong = num(g("LONG")); if (num(g("LONG")) >= 40) { const qbKey = Object.keys(players).find((k) => players[k].team === T && players[k].att > 0); if (qbKey) players[qbKey].long = Math.max(players[qbKey].long || 0, num(g("LONG"))); } }
            if (cat.name === "fumbles") { rec.fl = num(g("LOST")); }
            if (cat.name === "kicking") { const [fm] = frac(g("FG")); const [xm] = frac(g("XP")); const long = num(g("LONG")); const [, fa] = frac(g("FG")); const [, xa] = frac(g("XP")); rec.fgpts = fm * 3 + (long >= 50 ? 2 : long >= 40 ? 1 : 0) - (fa - fm) * 2 - (xa - xm); rec.xp = xm; rec.fgm = fm; rec.long = long; }
        ...
        Object.keys(teamDef).forEach((t) => { const d = teamDef[t]; const pa = d.pa; const paPts = pa === 0 ? 14 : pa <= 6 ? 10 : pa <= 13 ? 7 : pa <= 20 ? 3 : pa <= 27 ? 0 : pa <= 34 ? -1 : -4; dst[t] = { ...d, pts: Math.round((d.sack + d.int * 2 + d.fr * 2 + d.td * 6 + paPts) * 100) / 100, live, done }; });
      } catch (err) { /* skip game */ }
    }));
